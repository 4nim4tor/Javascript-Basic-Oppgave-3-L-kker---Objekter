...
console.log("The combined age of eveyone is " + combinedAge);
...
function diceRoller(diceCount, diceSize) {
  const diceNum = [];
  for (let i = 0; i < diceCount; i++) {
    const randomNumber = Math.ceil(Math.random() * diceSize);
    diceNum.push(randomNumber);
  }
  return diceNum;
}

const myDiceRoll = diceRoller(5, 20);
...
function textCleanUp() {
  const textCleaner = [];
  for (let text of messyText) {
    text = text.trim().toLowerCase();
    textCleaner.push(text);
  }
  return textCleaner;
}

const cleanText = textCleanUp();
